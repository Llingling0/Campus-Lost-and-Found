...
router.post('/', auth(false), (req, res) => {
  ...
  const safeLocation = sanitize(occurLocation || '');
  const safeDetail = sanitize(detail || '');
  const safeContact = sanitize(contactInfo || '');
  const safeDeposit = sanitize(depositLocation || '');
  ...
  db.prepare(`INSERT INTO item_posts(post_no, post_type, title, category_id, occur_time, occur_location, detail, contact_info, reward_amount, deposit_location, publisher_id, status, created_at, updated_at)
    VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(genNo('P'), postType, safeTitle, categoryId || null, occurTime || null, safeLocation, safeDetail, safeContact, rewardAmount || 0, safeDeposit, req.user.id, status, now, now);
  ...
  if (!isDraft) {
    notify(req.user.id, 1, post.id, '提交成功', `你发布的信息【${title}】已提交，请等待管理员审核`);
  }
...
router.put('/:id', auth(false), (req, res) => {
  ...
  db.prepare(`UPDATE item_posts SET title=?, category_id=?, occur_time=?, occur_location=?, detail=?, contact_info=?, reward_amount=?, deposit_location=?, updated_at=? WHERE id=?`)
    .run(sanitize(title), categoryId, occurTime, sanitize(occurLocation), sanitize(detail), sanitize(contactInfo), rewardAmount, sanitize(depositLocation), nowIso(), id);
...
router.post('/:id/submit', auth(false), (req, res) => {
  ...
  notify(post.publisher_id, 1, post.id, '提交成功', `你发布的信息【${post.title}】已提交，请等待管理员审核`);
  return ok(res, null, '已提交审核');
});

router.get('/', (req, res) => {
  let { postType, keyword = '', categoryId, status = 2, pageNo = 1, pageSize = 10 } = req.query;
  pageSize = Math.min(Math.max(Number(pageSize) || 10, 1), 50);
  const where = ['1=1'];
  const params = [];
...
router.get('/search/smart', (req, res) => {
  let { keyword = '', postType, categoryId, occurLocation, startTime, endTime, pageNo = 1, pageSize = 10 } = req.query;
  pageSize = Math.min(Math.max(Number(pageSize) || 10, 1), 50);
...
// 失物归还 — 发布者标记自己的失物帖子为"已归还"
router.post('/:id/return', auth(false), (req, res) => {
  const row = db.prepare('SELECT * FROM item_posts WHERE id=?').get(req.params.id);
  if (!row) return fail(res, '信息不存在');
  if (row.publisher_id !== req.user.id) return fail(res, '无权限');
  if (row.post_type !== 1) return fail(res, '仅失物信息支持归还操作');
  if (row.status !== 2) return fail(res, '当前状态不可归还');
  db.prepare('UPDATE item_posts SET status=6, updated_at=? WHERE id=?').run(nowIso(), row.id);
  return ok(res, null, '已标记为归还');
});
