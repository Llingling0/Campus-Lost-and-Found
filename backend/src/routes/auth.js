...
import { auth, signToken, recordLoginAttempt, isLockedOut, getRemainingLockMinutes } from '../middleware/auth.js';
...
router.post('/login', async (req, res) => {
  ...
  const loginIdentifier = `login_${identityType}_${identityNo}`;
  if (isLockedOut(loginIdentifier)) {
    const remaining = getRemainingLockMinutes(loginIdentifier);
    return fail(res, `账号已锁定，请${remaining}分钟后重试`);
  }
...
router.post('/admin-login', (req, res) => {
  ...
  const loginIdentifier = `admin_${username}`;
  if (isLockedOut(loginIdentifier)) {
    const remaining = getRemainingLockMinutes(loginIdentifier);
    return fail(res, `账号已锁定，请${remaining}分钟后重试`);
  }
