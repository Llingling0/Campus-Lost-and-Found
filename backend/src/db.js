...
function migrate() {
  // Add updated_at to feedbacks if missing
  const fbCols = db.prepare("PRAGMA table_info('feedbacks')").all();
  if (!fbCols.find(c => c.name === 'updated_at')) {
    db.prepare("ALTER TABLE feedbacks ADD COLUMN updated_at TEXT").run();
  }
  // Add image_url to announcements if missing
  const annCols = db.prepare("PRAGMA table_info('announcements')").all();
  if (!annCols.find(c => c.name === 'image_url')) {
...
function seedData() {
  ...
  const count = db.prepare('SELECT COUNT(*) as c FROM item_categories').get().c;
  if (count === 0) {
...
export function genNo(prefix) {
  return `${prefix}${Date.now()}${uuidv4().slice(0, 6).toUpperCase()}`;
}
